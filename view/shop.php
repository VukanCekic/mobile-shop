<?php require_once("shared/header.php") ?>
<?php
if (session_status() === PHP_SESSION_ACTIVE) {
  session_destroy();
}
?>
<!--Shop-->
<section id="shop font-roboto">
  <div class="container mt-3 py-5">
    <h3>Our Products</h3>
    <p>Here you can check out our products</p>
  </div>
  <div class="row mx-auto container-fluid">

    <div class="product text-center col-lg-3 col-md-4 col-sm-12">
      <img class="img-fluid mb-3" src="../assets/images/clothes/clothes1.jpeg" />
      <div class="star">
        <i class="fas fa-star"></i>
        <i class="fas fa-star"></i>
        <i class="fas fa-star"></i>
        <i class="fas fa-star"></i>
        <i class="fas fa-star"></i>
      </div>
      <h5 class="p-name">Product name</h5>
      <h4 class="p-price">Product price</h4>
      <a href="#"><button class="buy-btn">Buy Now</button></a>
    </div>
    <div class="product text-center col-lg-3 col-md-4 col-sm-12">
      <img class="img-fluid mb-3" src="../assets/images/clothes/clothes2.jpeg" />
      <div class="star">
        <i class="fas fa-star"></i>
        <i class="fas fa-star"></i>
        <i class="fas fa-star"></i>
        <i class="fas fa-star"></i>
        <i class="fas fa-star"></i>
      </div>
      <h5 class="p-name">Product name</h5>
      <h4 class="p-price">Product price</h4>
      <a href="#"><button class="buy-btn">Buy Now</button></a>
    </div>
    <div class="product text-center col-lg-3 col-md-4 col-sm-12">
      <img class="img-fluid mb-3" src="../assets/images/clothes/clothes3.jpeg" />
      <div class="star">
        <i class="fas fa-star"></i>
        <i class="fas fa-star"></i>
        <i class="fas fa-star"></i>
        <i class="fas fa-star"></i>
        <i class="fas fa-star"></i>
      </div>
      <h5 class="p-name">Product name</h5>
      <h4 class="p-price">Product price</h4>
      <a href="#"><button class="buy-btn">Buy Now</button></a>
    </div>
    <div class="product text-center col-lg-3 col-md-4 col-sm-12">
      <img class="img-fluid mb-3" src="../assets/images/clothes/clothes4.jpeg" />
      <div class="star">
        <i class="fas fa-star"></i>
        <i class="fas fa-star"></i>
        <i class="fas fa-star"></i>
        <i class="fas fa-star"></i>
        <i class="fas fa-star"></i>
      </div>
      <h5 class="p-name">Product name</h5>
      <h4 class="p-price">Product price</h4>
      <a href="#"><button class="buy-btn">Buy Now</button></a>
    </div>

    <div class="product text-center col-lg-3 col-md-4 col-sm-12">
      <img class="img-fluid mb-3" src="../assets/images/clothes/clothes1.jpeg" />
      <div class="star">
        <i class="fas fa-star"></i>
        <i class="fas fa-star"></i>
        <i class="fas fa-star"></i>
        <i class="fas fa-star"></i>
        <i class="fas fa-star"></i>
      </div>
      <h5 class="p-name">Product name</h5>
      <h4 class="p-price">Product price</h4>
      <a href="#"><button class="buy-btn">Buy Now</button></a>
    </div>
    <div class="product text-center col-lg-3 col-md-4 col-sm-12">
      <img class="img-fluid mb-3" src="../assets/images/clothes/clothes2.jpeg" />
      <div class="star">
        <i class="fas fa-star"></i>
        <i class="fas fa-star"></i>
        <i class="fas fa-star"></i>
        <i class="fas fa-star"></i>
        <i class="fas fa-star"></i>
      </div>
      <h5 class="p-name">Product name</h5>
      <h4 class="p-price">Product price</h4>
      <a href="#"><button class="buy-btn">Buy Now</button></a>
    </div>
    <div class="product text-center col-lg-3 col-md-4 col-sm-12">
      <img class="img-fluid mb-3" src="../assets/images/clothes/clothes3.jpeg" />
      <div class="star">
        <i class="fas fa-star"></i>
        <i class="fas fa-star"></i>
        <i class="fas fa-star"></i>
        <i class="fas fa-star"></i>
        <i class="fas fa-star"></i>
      </div>
      <h5 class="p-name">Product name</h5>
      <h4 class="p-price">Product price</h4>
      <a href="#"><button class="buy-btn">Buy Now</button></a>
    </div>
    <div class="product text-center col-lg-3 col-md-4 col-sm-12">
      <img class="img-fluid mb-3" src="../assets/images/clothes/clothes4.jpeg" />
      <div class="star">
        <i class="fas fa-star"></i>
        <i class="fas fa-star"></i>
        <i class="fas fa-star"></i>
        <i class="fas fa-star"></i>
        <i class="fas fa-star"></i>
      </div>
      <h5 class="p-name">Product name</h5>
      <h4 class="p-price">Product price</h4>
      <a href="#"><button class="buy-btn">Buy Now</button></a>
    </div>

    <div class="product text-center col-lg-3 col-md-4 col-sm-12">
      <img class="img-fluid mb-3" src="../assets/images/clothes/clothes1.jpeg" />
      <div class="star">
        <i class="fas fa-star"></i>
        <i class="fas fa-star"></i>
        <i class="fas fa-star"></i>
        <i class="fas fa-star"></i>
        <i class="fas fa-star"></i>
      </div>
      <h5 class="p-name">Product name</h5>
      <h4 class="p-price">Product price</h4>
      <a href="#"><button class="buy-btn">Buy Now</button></a>
    </div>
    <div class="product text-center col-lg-3 col-md-4 col-sm-12">
      <img class="img-fluid mb-3" src="../assets/images/clothes/clothes2.jpeg" />
      <div class="star">
        <i class="fas fa-star"></i>
        <i class="fas fa-star"></i>
        <i class="fas fa-star"></i>
        <i class="fas fa-star"></i>
        <i class="fas fa-star"></i>
      </div>
      <h5 class="p-name">Product name</h5>
      <h4 class="p-price">Product price</h4>
      <a href="#"><button class="buy-btn">Buy Now</button></a>
    </div>
    <div class="product text-center col-lg-3 col-md-4 col-sm-12">
      <img class="img-fluid mb-3" src="../assets/images/clothes/clothes3.jpeg" />
      <div class="star">
        <i class="fas fa-star"></i>
        <i class="fas fa-star"></i>
        <i class="fas fa-star"></i>
        <i class="fas fa-star"></i>
        <i class="fas fa-star"></i>
      </div>
      <h5 class="p-name">Product name</h5>
      <h4 class="p-price">Product price</h4>
      <a href="#"><button class="buy-btn">Buy Now</button></a>
    </div>
    <div class="product text-center col-lg-3 col-md-4 col-sm-12">
      <img class="img-fluid mb-3" src="../assets/images/clothes/clothes4.jpeg" />
      <div class="star">
        <i class="fas fa-star"></i>
        <i class="fas fa-star"></i>
        <i class="fas fa-star"></i>
        <i class="fas fa-star"></i>
        <i class="fas fa-star"></i>
      </div>
      <h5 class="p-name">Product name</h5>
      <h4 class="p-price">Product price</h4>
      <a href="#"><button class="buy-btn">Buy Now</button></a>
    </div>

    <nav aria-label="Page navigation example" class="mx-auto">
      <ul class="pagination  mx-auto">
        <li class="page-item">
          <a class="page-link color-primary" href="#">Previous</a>
        </li>
        <li class="page-item"><a class="page-link color-primary" href="?page_no=1">1</a></li>
        <li class="page-item"><a class="page-link color-primary" href="?page_no=2">2</a></li>
        <li class="page-item"><a class="page-link color-primary" href="#">...</a></li>
        <li class="page-item"><a class="page-link color-primary" href="#">5</a></li>

        <li class="page-item">
          <a class="page-link color-primary" href="#">Next</a>
        </li>
      </ul>
    </nav>
  </div>
</section>

<?php require_once("shared/footer.php") ?>